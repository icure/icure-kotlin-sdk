...
class AnonymousAccessApi(basePath: kotlin.String = defaultBasePath, webClient: WebClient = NettyWebClient(), authHeader: String? = null) : ApiClient(basePath, webClient, authHeader) {
    ...
    fun getAvailabilitiesByPeriodAndCalendarItemTypeIdRequestConfig(
        ...
        return RequestConfig(
            ...
            path = "/rest/v2/aa/available/inGroup/{groupId}/forUser/{userId}/type/{calendarItemTypeId}".replace(
                "{" + "groupId" + "}",
                "${URLEncoder.encode(groupId.toString(), Charsets.UTF_8)}"
            ).replace("{" + "userId" + "}", "${URLEncoder.encode(userId.toString(), Charsets.UTF_8)}").replace(
                "{" + "calendarItemTypeId" + "}",
                "${URLEncoder.encode(calendarItemTypeId.toString(), Charsets.UTF_8)}"
            ),
    ...
    suspend fun listAppointmentTypesForUser(
        ...
        startDate: kotlin.Long,
        endDate: kotlin.Long
    ): kotlin.collections.List<AppointmentTypeAndPlaceDto> {
