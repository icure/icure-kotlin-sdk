...
suspend fun DocumentApi.createDocument(user: UserDto, document: DocumentDto, config: CryptoConfig<DocumentDto>) =
    this.createDocument(
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.findByTypeHCPartyMessageSecretFKeys(user: UserDto, documentTypeCode: String, hcPartyId: String, secretFKeys: String, config: CryptoConfig<DocumentDto>) : List<DocumentDto>? {
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.findWithoutDelegation(user: UserDto, limit: Int?, config: CryptoConfig<DocumentDto>) : List<DocumentDto>? {
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.getDocument(user: UserDto, documentId: String, config: CryptoConfig<DocumentDto>): DocumentDto?  {
    return this.getDocument(documentId)?.let { config.decryptDocument(user.healthcarePartyId!!, it) }
}

@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.modifyDocument(user: UserDto, document: DocumentDto, config: CryptoConfig<DocumentDto>) : DocumentDto?  {
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.modifyDocuments(user: UserDto, documents: List<DocumentDto>, config: CryptoConfig<DocumentDto>) : List<DocumentDto>?  {
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.deleteAttachment(user: UserDto, documentId: String, config: CryptoConfig<DocumentDto>) : DocumentDto? {
...
@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.getDocumentsByExternalUuid(user: UserDto, externalUuid: String, config: CryptoConfig<DocumentDto>): List<DocumentDto>?  {
    return this.getDocumentsByExternalUuid(externalUuid)?.map { config.decryptDocument(user.healthcarePartyId!!, it) }
}

@ExperimentalCoroutinesApi
@ExperimentalStdlibApi
suspend fun DocumentApi.getDocuments(user: UserDto, listOfIdsDto: ListOfIdsDto, config: CryptoConfig<DocumentDto>): List<DocumentDto>?  {
    return this.getDocuments(listOfIdsDto)?.map { config.decryptDocument(user.healthcarePartyId!!, it) }
}

suspend fun CryptoConfig<DocumentDto>.encryptDocument(myId: String, delegations: Set<String>, document: DocumentDto): io.icure.kraken.client.models.DocumentDto {
    return if (document.encryptionKeys.any { (_,s) -> s.isNotEmpty() }) {
        ...
        document.copy(encryptionKeys = (delegations + myId).fold(document.encryptionKeys) { m, d ->
            m + (d to setOf(DelegationDto(listOf(), myId, d, this.crypto.encryptKeyForHcp(myId, d, document.id, secret))))
        })
    }.let { p ->
        ...
        DocumentMapperFactory.instance.map(sanitizedDocument.copy(encryptedSelf = Base64.getEncoder().encodeToString(encryptAES(data = marshalledData, key = key))))
    }
}

suspend fun CryptoConfig<DocumentDto>.decryptDocument(myId: String, document: io.icure.kraken.client.models.DocumentDto): DocumentDto = DocumentMapperFactory.instance.map(document).let { p ->
    val key = this.crypto.decryptEncryptionKeys(myId, p.encryptionKeys).firstOrNull()?.let { aesKey ->
        aesKey.replace(
            "-",
            ""
