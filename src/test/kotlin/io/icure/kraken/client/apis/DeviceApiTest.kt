...
class DeviceApiTest() {
    ...
    @ParameterizedTest
    @MethodSource("fileNames") // six numbers
	fun getDeviceHcPartyKeysForDelegateTest(fileName: String) = runBlocking {

        if (TestUtils.skipEndpoint(fileName, "getDeviceHcPartyKeysForDelegate")) {
            assertTrue(true, "Test of getDeviceHcPartyKeysForDelegate endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "getDeviceHcPartyKeysForDelegate")
                val deviceId: kotlin.String = TestUtils.getParameter<kotlin.String>(fileName, "getDeviceHcPartyKeysForDelegate.deviceId")!!.let {
                    (it as? DeviceDto)?.takeIf { TestUtils.isAutoRev(fileName, "getDeviceHcPartyKeysForDelegate") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getDevice(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                    } as? kotlin.String ?: it
                ...
                val response = api(credentialsFile).getDeviceHcPartyKeysForDelegate(deviceId = deviceId)

                val testFileName = "DeviceApi.getDeviceHcPartyKeysForDelegate"
                val file = File(workingFolder + File.separator + this::class.simpleName + File.separator + fileName, "$testFileName.json")
                try {
                    ...
                    assertAreEquals("getDeviceHcPartyKeysForDelegate", objectFromFile, response)
                    println("Comparison successful")
                }
                catch (e: Exception) {
                    when (e) {
