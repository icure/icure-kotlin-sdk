...
class DocumentApiTest() {
    ...
    fun api(fileName: String) = DocumentApi(basePath = java.lang.System.getProperty("API_URL"), authProvider = fileName.basicAuth())
	...
	fun deleteAttachmentTest(fileName: String) = runBlocking {
        ...
        if (TestUtils.skipEndpoint(fileName, "deleteAttachment")) {
            ...
            try{
                ...
                val rev: kotlin.String = TestUtils.getParameter<kotlin.String>(fileName, "deleteAttachment.rev")!!.let {
                    (it as? DocumentDto)?.takeIf { TestUtils.isAutoRev(fileName, "deleteAttachment") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getDocument(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                ...
                val response = api(credentialsFile).deleteAttachment(documentId = documentId, rev = rev)
