...
class PermissionApiTest() {
    ...
    companion object {
        ...
        @JvmStatic
        fun fileNames() = listOf("PermissionApi.json")
    }

    // http://127.0.0.1:16043
    fun api(fileName: String) = PermissionApi(basePath = java.lang.System.getProperty("API_URL"), authHeader = fileName.basicAuth())
                ...
                bodies.forEach {body ->
                    ...
                    val parameters = createFunction!!.parameters.mapNotNull {
                        when(it.type.javaType) {
                            PermissionDto::class.java -> it to objectMapper.convertValue(body, PermissionDto::class.java)
                            PermissionApi::class.java -> it to api(credentialsFile)
                            else -> null
                        }
                    }.toMap()
	...
	fun modifyUserPermissionsTest(fileName: String) = runBlocking {

        if (TestUtils.skipEndpoint(fileName, "modifyUserPermissions")) {
            assertTrue(true, "Test of modifyUserPermissions endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "modifyUserPermissions")
                val userId: kotlin.String = TestUtils.getParameter<kotlin.String>(fileName, "modifyUserPermissions.userId")!!.let {
                    (it as? PermissionDto)?.takeIf { TestUtils.isAutoRev(fileName, "modifyUserPermissions") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getPermission(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                    } as? kotlin.String ?: it
                    }
                val permissionDto: PermissionDto = TestUtils.getParameter<PermissionDto>(fileName, "modifyUserPermissions.permissionDto")!!.let {
                    (it as? PermissionDto)?.takeIf { TestUtils.isAutoRev(fileName, "modifyUserPermissions") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getPermission(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                ...
                val response = api(credentialsFile).modifyUserPermissions(userId,permissionDto)

                val testFileName = "PermissionApi.modifyUserPermissions"
                val file = File(workingFolder + File.separator + this::class.simpleName + File.separator + fileName, "$testFileName.json")
                try {
                    val objectFromFile = (response as? Flow<ByteBuffer>)?.let { file.readAsFlow() } ?: objectMapper.readValue(file,  if (response as? List<PermissionDto>? != null) {
                        if ("kotlin.collections.List<PermissionDto>".contains("String>")) {
                            object : TypeReference<List<String>>() {}
                        } else {
                            object : TypeReference<List<PermissionDto>>() {}
                        }
                    } else if(response as? kotlin.collections.Map<String, String>? != null){
                        object : TypeReference<Map<String,String>>() {}
                    } else {
                        object : TypeReference<kotlin.collections.List<PermissionDto>>() {}
                    })
                    assertAreEquals("modifyUserPermissions", objectFromFile, response)
