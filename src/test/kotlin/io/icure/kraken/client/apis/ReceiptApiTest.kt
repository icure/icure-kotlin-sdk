                ...
                if (receiptDto as? Collection<*> == null) {
                    receiptDto.also {
                        if (TestUtils.isAutoRev(fileName, "createReceipt") && it != null) {
                            ...
                            val rev = object :
                                TypeReference<ReceiptDto>() {}.type::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                                .first { it.name == "rev" }
                            rev.setter.call(it, currentRev)
                        }
                    }
                } else {
                    val paramAsCollection =
                        receiptDto as? Collection<ReceiptDto> ?: emptyList<ReceiptDto>() as Collection<ReceiptDto>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "createReceipt") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }

                            val currentRev = api(credentialsFile).getReceipt(id.getter.call(it) as String).rev
                            val rev = it::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                                .first { it.name == "rev" }
                            rev.setter.call(it, currentRev)
                        }
                    }
                ...
                try {
                    val objectFromFile =
                        (response as? Flow<ByteBuffer>)?.let { file.readAsFlow() } ?: objectMapper.readValue(file,
                            if (response as? List<ReceiptDto>? != null) {
                    ...
                    file.createNewFile()
                    (response as? Flow<ByteBuffer>)?.let { it.writeToFile(file) } ?: objectMapper.writeValue(
                        file,
                        response
                    )
                    assert(true)
                ...
                if (listOfIdsDto as? Collection<*> == null) {
                    listOfIdsDto.also {
                        if (TestUtils.isAutoRev(fileName, "deleteReceipts") && it != null) {
                            ...
                            val currentRev = api(credentialsFile).getReceipt(id.getter.call(it) as String).rev
                            val rev = object :
                                TypeReference<ListOfIdsDto>() {}.type::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                                .first { it.name == "rev" }
                            rev.setter.call(it, currentRev)
                        }
                    }
                } else {
                    val paramAsCollection = listOfIdsDto as? Collection<ListOfIdsDto>
                        ?: emptyList<ListOfIdsDto>() as Collection<ListOfIdsDto>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "deleteReceipts") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
                ...
                if (receiptId as? Collection<*> == null) {
                    receiptId.also {
                        if (TestUtils.isAutoRev(fileName, "getReceipt") && it != null) {
                            ...
                            val currentRev = api(credentialsFile).getReceipt(id.getter.call(it) as String).rev
                            val rev = object :
                                TypeReference<kotlin.String>() {}.type::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                                .first { it.name == "rev" }
                            rev.setter.call(it, currentRev)
                        }
                    }
                } else {
                    val paramAsCollection = receiptId as? Collection<kotlin.String>
                        ?: emptyList<kotlin.String>() as Collection<kotlin.String>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "getReceipt") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
        ...
        try {
            ...
            if (TestUtils.skipEndpoint(fileName, "getReceiptAttachment")) {
                ...
                val attachmentId: kotlin.String =
                    TestUtils.getParameter(fileName, "getReceiptAttachment.attachmentId")!!
                if (attachmentId as? Collection<*> == null) {
                    ...
                    val paramAsCollection = attachmentId as? Collection<kotlin.String>
                        ?: emptyList<kotlin.String>() as Collection<kotlin.String>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "getReceiptAttachment") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
                ...
                if (enckeys as? Collection<*> == null) {
                    ...
                    val paramAsCollection =
                        enckeys as? Collection<kotlin.String> ?: emptyList<kotlin.String>() as Collection<kotlin.String>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "getReceiptAttachment") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
        ...
        try {
            ...
            if (TestUtils.skipEndpoint(fileName, "listByReference")) {
                ...
                if (ref as? Collection<*> == null) {
                    ...
                    val paramAsCollection =
                        ref as? Collection<kotlin.String> ?: emptyList<kotlin.String>() as Collection<kotlin.String>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "listByReference") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
        ...
        try {
            ...
            if (TestUtils.skipEndpoint(fileName, "setReceiptAttachment")) {
                ...
                if (blobType as? Collection<*> == null) {
                    ...
                    val paramAsCollection = blobType as? Collection<kotlin.String>
                        ?: emptyList<kotlin.String>() as Collection<kotlin.String>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "setReceiptAttachment") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
                ...
                if (body as? Collection<*> == null) {
                    body.also {
                        if (TestUtils.isAutoRev(fileName, "setReceiptAttachment") && it != null) {
                            ...
                            val currentRev = api(credentialsFile).getReceipt(id.getter.call(it) as String).rev
                            val rev = object :
                                TypeReference<kotlinx.coroutines.flow.Flow<java.nio.ByteBuffer>>() {}.type::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                                .first { it.name == "rev" }
                            rev.setter.call(it, currentRev)
                        }
                    }
                } else {
                    val paramAsCollection = body as? Collection<kotlinx.coroutines.flow.Flow<java.nio.ByteBuffer>>
                        ?: emptyList<kotlinx.coroutines.flow.Flow<java.nio.ByteBuffer>>() as Collection<kotlinx.coroutines.flow.Flow<java.nio.ByteBuffer>>
                    paramAsCollection.forEach {
                        if (TestUtils.isAutoRev(fileName, "setReceiptAttachment") && it != null) {
                            val id = it::class.memberProperties.first { it.name == "id" }
                ...
                if (functionName.startsWith("create") || functionName.startsWith("new")) { // new
                    for (fileElement in objectFromFile) {
                        fileElement::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                            .firstOrNull { it.name == "id" }?.setter?.call(fileElement, null)
                        fileElement::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                    ...
                    for (responseElement in iterableResponse) {
                        responseElement::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
                            .firstOrNull { it.name == "id" }?.setter?.call(responseElement, null)
                        responseElement::class.memberProperties.filterIsInstance<KMutableProperty<*>>()
